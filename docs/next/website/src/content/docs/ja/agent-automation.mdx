...
| サーバーで保護された操作（現在は中断）を実行する | 現在の `AgentInfo.actions` capability を使う Raw API `agent.perform_action` |
...
ダッシュボードやタッチ操作では、`blocked` / `working` 状態から推測して `agent send-keys` を呼び出すのではなく、`AgentInfo.actions` の型付き capability を優先してください。`agent.perform_action` が受け取るのは opaque capability ID だけです。Herdr は canonical key を 1 回送る前に、正確なプロセス、ターミナル／ペイン identity、状態、manifest evidence、screen fingerprint を再検証します。capability は短時間だけ有効で、1 回しか使用できません。capability がない場合、より広い lifecycle status が `blocked` または `working` でも、現在の UI はその操作向けに確実には分類されていません。approval は予約済みですが、one-time approval として現在の prompt を検証できるまでは意図的に利用できません。現時点では、明示的に表示された interrupt footer だけが対象です。
