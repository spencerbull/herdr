...
| Perform a server-guarded action (currently interrupt) | Raw API `agent.perform_action` using the current `AgentInfo.actions` capability |
...
For a dashboard or touch control, prefer the typed capabilities in
`AgentInfo.actions` over guessing from `blocked`/`working` status and calling
`agent send-keys`. `agent.perform_action` accepts only the opaque capability ID;
Herdr revalidates the exact process, terminal/pane identity, state, manifest
evidence, and screen fingerprint before sending one canonical key. Capabilities
are short-lived and single-use. Their absence means the current UI is not
positively classified for that action, even if the broader lifecycle status is
`blocked` or `working`. Approval is reserved but intentionally unavailable until
a current prompt can be verified as one-time approval; only an explicit visible
interrupt footer currently qualifies.
