...
redraw or state change. It closes the client/API time-of-check gap, but PTY
output can still race the final queue write; clients should describe the action
as perform-time revalidated, not transactionally atomic. Refresh agent state and
wait for a newly issued capability instead of retrying a rejected or failed ID.
