...
identity, foreground agent process, semantic status, state-change sequence,
revision, manifest evidence, detection-content sequence, and a fingerprint of
the screen plus OSC evidence. It then sends exactly one server-selected key:
Enter for `approve` or Escape for `interrupt`. The request cannot choose a
target, action, or raw key payload.
...
current on-screen `esc to interrupt` working footer; a spinner or lifecycle
