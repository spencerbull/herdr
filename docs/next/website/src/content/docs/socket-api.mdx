...
- list, inspect, read, prompt, wait on, rename, focus, start, attach, and perform guarded agent actions
...
| Agent | `agent.list`, `agent.order.get`, `agent.order.set`, `agent.get`, `agent.read`, `agent.explain`, `agent.send_keys`, `agent.perform_action`, `agent.prompt`, `agent.wait`, `agent.rename`, `agent.focus`, `agent.start`, `agent.view.set`, `agent.view.clear` |
...
Read or set the durable ordering shared by Herdr's built-in Agents view and API
clients. The public values are `grouped` and `priority`; grouped maps to the
persisted `ui.agent_panel_sort = "spaces"` setting.

```json
{"id":"order_get","method":"agent.order.get","params":{}}
{"id":"order_set","method":"agent.order.set","params":{"order":"priority"}}
```

`agent.order.set` is idempotent, persists the setting, and applies it to the
running Herdr UI before returning. Clients can poll `agent.order.get` alongside
their normal agent snapshot refresh to observe changes made from either UI.
...
`agent.get`, `agent.list`, and `session.snapshot` can expose an `actions` array
on an agent record. Each entry is a short-lived capability for one server-owned
action:
...
```json
{
  "actions": [{
    ...
    "state_change_seq": 42,
    "revision": 3,
    "expires_at_unix_ms": 1785384000000,
    "evidence": {
      "manifest_source": "bundled",
      "manifest_version": "2026.07.18.1",
      "rule_id": "screen_working_fallback"
    }
  }]
}
```

Perform the action by returning only its opaque capability ID:

```json
{
  "id": "req_action",
  "method": "agent.perform_action",
  "params": { "capability_id": "act_..." }
}
```

Capabilities are process-memory-only, expire after 30 seconds, and are consumed
before revalidation and input. A consumed capability is never retried, including
when input could not be queued. A server restart or live handoff invalidates all
previous capabilities. Unknown, expired, consumed, and previous-server IDs all
return `agent_action_capability_not_found`.

The server re-resolves and rechecks the terminal, current pane/workspace/tab
identity, exact foreground agent process instance (matched PID, process
start/creation identity, and process group where the platform provides one),
semantic status, state-change sequence, revision, trusted bundled-manifest
evidence, detection-content sequence, and a fingerprint of the screen plus OSC
evidence. It then sends exactly one server-selected key: Enter for `approve` or
Escape for `interrupt`. The request cannot choose a target, action, or raw key
payload. Guarded actions use one canonical byte (`CR` or `ESC`) independent of
terminal keyboard protocol.

The guarded write does not use the ordinary asynchronous input queue. Herdr
accepts it only when no earlier user input remains queued, blocks later user
input from interposing, consumes already-readable PTY output before and after
validation, revalidates the exact process and a coherent screen/OSC snapshot in
the PTY writer, and reports success only after the one-byte write succeeds. A
backpressured write is cancelled after a short deadline and cannot be written
later after an error response. Guarded capabilities are currently withheld on
Windows because the synchronous ConPTY writer cannot be cancelled safely; Herdr
does not fall back to the ordinary queue.

`approve` is reserved in the typed action enum, but Herdr intentionally does not
issue approval capabilities yet. Current prompt families do not provide a
verified, action-specific screen rule that proves Enter means one-time approval;
generic confirmations, structured questions, selected permission policies, and
weak `[y/n]` prompts are not sufficient. An `interrupt` capability requires the
exact trusted bundled rule for the current on-screen `esc to interrupt` working
footer; local overrides, downloaded manifests, a spinner, or lifecycle status
alone do not qualify.

Revalidation is intentionally conservative and can reject an action after any
redraw, state change, process replacement, queued input, or writer
backpressure. A same-kind replacement process cannot inherit the previous
process's screen: on the first process observation and every replacement, Herdr
drains buffered PTY output, rechecks the exact process, and only then records the
detection baseline. Trusted evidence already visible at that boundary must
disappear and later reappear before a capability can be issued. Evidence first
painted after an evidence-free boundary can qualify normally. Only actual PTY
output advances this process-evidence freshness; a local resize or reflow can
invalidate an existing capability but cannot make inherited evidence fresh. An
in-place `exec` that preserves PID and process-start identity is process
continuity, not a detected replacement boundary. Environment hints can identify
an agent for presentation, but cannot establish the exact executable identity
required by a guarded action.

The remaining race is the final writer-boundary validation return to the
one-byte PTY write syscall; the external process can still change in that
interval, so clients should describe the action as writer-boundary revalidated,
not transactionally atomic. Refresh agent state and wait for a newly issued
capability instead of retrying a rejected or failed ID.
