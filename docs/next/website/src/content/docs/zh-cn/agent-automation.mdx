...
| 执行服务器保护的操作（当前为中断） | 使用当前 `AgentInfo.actions` capability 的原始 API `agent.perform_action` |
...
对于仪表板或触控操作，应优先使用 `AgentInfo.actions` 中的类型化 capability，而不是根据 `blocked` / `working` 状态猜测后调用 `agent send-keys`。`agent.perform_action` 只接受 opaque capability ID；Herdr 会在发送一次 canonical key 前重新验证确切的进程、终端／窗格 identity、状态、manifest evidence 和 screen fingerprint。capability 有效期很短且只能使用一次。缺少 capability 表示当前 UI 尚未被明确分类为可执行该操作，即使更宽泛的 lifecycle status 是 `blocked` 或 `working`。approval 已预留，但在当前 prompt 能被验证为 one-time approval 之前会有意保持不可用；目前只有明确可见的 interrupt footer 符合条件。
